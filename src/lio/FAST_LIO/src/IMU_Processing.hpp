...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  // ===== MID360 倾斜安装修正：只用 mean_acc 计算对齐姿态，不要覆盖 mean_acc =====
  // 注意：mean_acc 是 IMU 初始化期间持续累计的原始加速度均值，
  // 不能写成 mean_acc = q_gravity_align * mean_acc，
  // 否则初始化时间越长，均值越容易被反复旋转污染。

  Eigen::Vector3d mean_acc_raw = mean_acc;

  Eigen::Quaterniond q_gravity_align = Eigen::Quaterniond::FromTwoVectors(
      mean_acc_raw, Eigen::Vector3d::UnitZ()
  );

  Eigen::Vector3d mean_acc_aligned = q_gravity_align * mean_acc_raw;

  state_ikfom init_state = kf_state.get_x();
  init_state.rot = q_gravity_align;
  init_state.grav = S2(- mean_acc_aligned / mean_acc_aligned.norm() * G_m_s2);
